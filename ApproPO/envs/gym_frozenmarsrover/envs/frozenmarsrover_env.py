...
class FrozenMarsRoverEnv(Env):
    ...
    def __init__(self, desc=None, map_name="4x4",is_slippery=False, type=None):
        ...
        # Gymnasium API: expose the feature vector the algorithms consume.
        ...
        # Store transition dynamics for manual step implementation
        self.P = P
        self.isd = isd
        self.nS = nS
        self.nA = nA
    ...
    def render(self, mode='human'):
        ...
        # Simple colorize without gym.utils
        desc[row][col] = f"\033[91m{desc[row][col]}\033[0m"
    ...
    def _state_repr(self, location):
        ...
        if self.nS == 16 or self.nS == 64: # Medium
            grid = np.zeros(self.nS, dtype=np.float32)
            grid[risk_zone] = 1.0
            grid[goal] = 2.0
            grid[location] = 3.0
            self.goal = goal
            self.risk_zone = risk_zone
        elif self.nS == 900: # Large
            grid = np.zeros((1, 1, 30, 30), dtype=np.float32)
            ...
            grid[0, 0, loc] = 3
        return np.array(grid / 3.0, dtype=np.float32)

    def get_env_attributes(self):
        if self.nS == 16: # Small
            risk_zones = [5, 7, 11, 12]
            goal = 15
        elif self.nS == 64: # Medium
    ...
    def step(self, action):
        ...
        # Use manually stored P dictionary
        transitions = self.P[self.s][action]
        probs = [transition[0] for transition in transitions]
        transition_idx = self.np_random.choice(len(transitions), p=probs)
        _, next_state, reward, done = transitions[transition_idx]
        ...
        return state, reward, done, False, {'goal': goal, 'constraint': constraint}

    def reset(self, seed=None, options=None):
        super().reset(seed=seed)
        self.s = self.np_random.choice(self.nS, p=self.isd)
        self.lastaction = None
        return self._state_repr(self.s), {}
