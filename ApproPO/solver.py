...
def run(proj_oracle=None, rl_oracle_generator=None, args=None):
    ...
    for episode in range(args.num_epochs):
        ...
        # Algorithm 4 in the paper updates the online learner every round using
        # the estimated measurement vector returned by the positive-response oracle.
        reset = False
        value = np.dot(theta, np.append(best_exp_rtn, args.mx_size))
        positive_response = value <= 0 or np.isclose(
            0, value, atol=args.atol_cache, rtol=args.rtol_cache
        )
        old_theta = theta
        ...
        shadow.add_response(best_exp_rtn=shadow_exp_rtn, traj_len=shadow_exp_stats['traj_len'], stats=stats)
        policy.add_response(best_exp_rtn=best_exp_rtn, traj_len=best_exp_stats['traj_len'], stats=stats)
        ...
        status += f'  theta: {old_theta[:2]} -> {theta[:2]} -- <theta,u>: {value}\n'
        if not positive_response:
            status += '  warning: positive-response oracle did not find a non-negative scalar reward\n'
